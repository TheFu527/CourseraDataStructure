// set_range_sum.cpp: 定义控制台应用程序的入口点。
//
...
void erase(int x) {
	...
	Vertex* eraseVertex = NULL;
	eraseVertex = find(root, x);
	if (eraseVertex == NULL || eraseVertex->key != x)
		return;
	//cout << eraseVertex->key << endl;
	if(eraseVertex->left!=NULL)
		eraseVertex->left->parent = NULL;
	if(eraseVertex->right!=NULL)
		eraseVertex->right->parent = NULL;
	root = merge(eraseVertex->left,eraseVertex->right);
}

bool find(int x) {
	// Implement find yourself
	Vertex* findVertex = NULL;
	findVertex = find(root, x);
	if (findVertex != NULL&&findVertex->key == x)
	{
		//cout << findVertex->key << endl;
		return true;
	}
	return false;
}
